  ...
  try {
    ...
    let html = `
      ...
      <div class="nw-banner">
        ...
        ${perDay != null && !_privacyMode ? `<div style="font-size:11px;color:var(--muted);margin-top:3px">${(perDay < 0 ? '-$' : '$') + Math.abs(perDay).toFixed(2)}/day · ${daysLeft} day${daysLeft !== 1 ? 's' : ''} left</div>` : ''}
      </div>
...
async function autoUpdateCryptoSnapshots(accounts, existingSnaps, holdingsByAcct) {
  ...
  try {
    ...
    const priceMap = {};
    tickers.forEach(t => { if (!priceMap[t.symbol.toUpperCase()]) priceMap[t.symbol.toUpperCase()] = t.quotes?.USD?.price || 0; });
...
async function loadWorkout(silent = false) {
  ...
  if (_workoutDay) {
    // ── Day view ──
    const dateStr = _workoutDay;
    try {
      const workouts = await api('GET', 'workouts', `user_id=eq.${currentUserId}&date=eq.${dateStr}&order=created_at.asc&select=*`);
      ...
      fab.onclick = () => openWorkoutModal(dateStr);
      fab.style.display = '';
    } catch(e) {
      el.innerHTML = `<div class="empty-state"><div class="empty-state-text">Error loading</div></div>`;
      showToast(e.message, 'error');
  ...
  try {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      dayCircles += `<div onclick="openWorkoutDay('${dStr}')" style="display:flex;flex-direction:column;align-items:center;gap:4px;cursor:pointer;-webkit-tap-highlight-color:transparent">
            ...
            html += `<div class="list-item" data-item-name="${item.name.toLowerCase()}">
              ...
              <div style="display:flex;gap:8px;align-items:center">
                <button id="cart-${item.id}" onclick="toggleGroceryToBuy('${item.id}',${item.to_buy})" style="background:none;border:none;cursor:pointer;padding:4px;display:flex">
                  <svg viewBox="0 0 24 24" width="22" height="22" stroke="${item.to_buy ? '#22c55e' : 'var(--muted)'}" fill="${item.to_buy ? 'rgba(34,197,94,0.15)' : 'none'}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="transition:stroke 0.15s,fill 0.15s"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
                </button>
